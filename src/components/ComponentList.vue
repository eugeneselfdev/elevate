            ...
            <table class="w-full text-sm text-left rtl:text-right text-body">
                ...
                <tbody>
                    <tr v-for="item in items" :key="item.id" class="bg-neutral-primary-soft border-b border-default hover:bg-neutral-secondary-medium flex flex-col md:table-row mb-4 md:mb-0">
                        ...
                        <td class="px-6 md:px-4 py-2 md:py-6 flex-1 md:table-cell  before:font-bold before:mr-2 before:block md:before:hidden font-bold">
                            <RouterLink :to="`/component/${item.id}`">
                                {{ item.na }}
                            </RouterLink>
                        </td>
